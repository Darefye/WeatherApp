...
class MainFragment : Fragment() {
    ...
    private fun updateCurrentCard() = with(binding) {
        model.lifeDataCurrent.observe(viewLifecycleOwner) {
            val maxMinStr = "${it.maxTemp}°C/${it.minTemp}°C"
            tvData.text = it.time
            tvCity.text = it.city
            tvCurrentTemp.text = it.currentTemp.ifEmpty { maxMinStr }
            tvCondition.text = it.condition
            tvMaxMin.text = if (it.currentTemp.isEmpty()) "" else maxMinStr

            ivWeather.load("https:" + it.imageUrl)
        }
    }
    ...
    private fun requestWeatherData(city: String) {
        ...
        val request = StringRequest(
            ...
            { error -> Log.d("LOG1", "Error from request$error") }
        )
        queue.add(request)
    }
    ...
    private fun parseWeatherData(result: String) {
        ...
        parseCurrentData(mainObject, list.first())
